...
function M.deploy_all(show_progress)
  ...
  for _, server in ipairs(config.servers) do
    rsync.deploy_to_server(config, server, project_root, function(srv, success, error_msg, duration)
      ...
      if show_progress then
        if success then
          vim.notify(string.format("✓ %s (%dms) [%d/%d]", srv.name, duration, completed, total), vim.log.levels.INFO)
        else
          vim.notify(string.format("✗ %s failed: %s [%d/%d]", srv.name, error_msg or "Unknown error", completed, total), vim.log.levels.ERROR)
        end
      end
...
function M.deploy_to_specific_server(server_name)
  ...
  rsync.deploy_to_server(config, server, project_root, function(srv, success, error_msg, duration)
    if success then
      ...
      vim.notify(string.format("✗ %s deployment failed: %s", srv.name, error_msg or "Unknown error"), vim.log.levels.ERROR)
    end
  end)
end
...
function M.deploy_current_file_to_all(show_progress)
  ...
  local rsync = require("deployment.rsync")
  for _, server in ipairs(config.servers) do
    rsync.deploy_single_file_to_server(config, server, project_root, relative_path, function(srv, success, error_msg, duration)
      completed = completed + 1
      ...
      if show_progress then
        if success then
          vim.notify(string.format("✓ %s deployed %s (%dms) [%d/%d]", srv.name, filename, duration, completed, total), vim.log.levels.INFO)
        else
          vim.notify(string.format("✗ %s failed to deploy %s: %s [%d/%d]", srv.name, filename, error_msg or "Unknown error", completed, total), vim.log.levels.ERROR)
        end
      end
...
function M.deploy_current_file_to_server(server_name)
  ...
  rsync.deploy_single_file_to_server(config, server, project_root, relative_path, function(srv, success, error_msg, duration)
    if success then
      vim.notify(string.format("✓ %s deployed %s successfully (%dms)", srv.name, filename, duration), vim.log.levels.INFO)
    else
      vim.notify(string.format("✗ %s deployment of %s failed: %s", srv.name, filename, error_msg or "Unknown error"), vim.log.levels.ERROR)
    end
  end)
end
...
function M.show_deployment_summary(results, show_summary)
  if not show_summary then return end
  ...
  local summary = string.format("Deployment complete: %d successful, %d failed (avg: %dms)", successful, failed, avg_time)
...
function M.show_file_deployment_summary(results, filename, show_summary)
  ...
  local avg_time = math.floor(total_time / #results)
  local summary = string.format("File %s deployment: %d successful, %d failed (avg: %dms)", filename, successful, failed, avg_time)
...
options:
  ...
  vim.notify("Created example " .. config_type .. " .deployment file at: " .. deployment_file.filename, vim.log.levels.INFO)
end
...
function M.list_configurations()
  local project_root = M.get_project_root()
  local parser = require("deployment.parser")
  local multi_config, err = parser.parse_multi_config_deployment(
